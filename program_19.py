...
def result(state, move):
	...
	stateCopy[1] = square(move, state[0]).bigSq
	return stateCopy
	...
	if (gameStage < threshold): #algorithm 1 (denial of diagonals)
		...
		if nextsquare != 9:
			...
			for temp in bias:
				...
				if square in vm:
					if (not isBoardWon(game[(square%9)*9:(square%9)*9+9])):
						return square
			temp = [topRightNum, topLeftNum, botLeftNum, botRightNum]
			while (len(temp)>0):
				if min(temp) == topRightNum and (not isBoardWon(game[18:27])):
					return 2
				elif min(temp) == topLeftNum and (not isBoardWon(game[0:9])):
					return 0
				elif min(temp) == botRightNum and (not isBoardWon(game[72:81])):
					return 8
				elif (not isBoardWon(game[54:63])):
					return 6
			...
			if (not isBoardWon(game[choice*9:choice*9+9])):
				for i in range (4):
					if (not isBoardWon(game[bias[i]*9:bias[i]*9+9])):
						return bias[i]
